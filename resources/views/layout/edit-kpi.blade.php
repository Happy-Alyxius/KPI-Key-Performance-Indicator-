    ...
    <div class="container-fluid">
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">Pengubahan Data KPI</h6>
            </div>
            <div class="card-body">
                <form action="{{route('update-kpi',$data['id'])}}" method="post">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <select name="jabatan" id="jabatan" class="form-control">
                            ...
                            <option value="{{$s1['id']}}" @if ($data['id_role'] == $s1['id']) selected @endif>{{$s1['name_role']}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select name="unit" id="unit" class="form-control">
                            ...
                            <option value="{{$s2['id']}}" @if ($data['id_role'] == $s2['id']) selected @endif >{{$s2['name_vocation']}}</option>
                            @endforeach
                        </select>
                    </div>
