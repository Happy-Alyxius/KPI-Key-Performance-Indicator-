    ...
    <div class="container-fluid">
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">Perubahan Data Pertanyaan KPI</h6>
            </div>
            <div class="card-body">
                <form action="{{route('update-pertanyaan',$data['id'])}}" method="post">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        <label>Bobot</label>
                        <input type="number" class="form-control @error('bobot') is-invalid @enderror" name="bobot" value="{{ $data['bobot'] }}" required>
                        @error('bobot')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
